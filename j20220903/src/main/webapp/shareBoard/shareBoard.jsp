...
<body>
	...
	<div class="a" >
		<button type="button" class="writebtn" 
			        onClick="location.href='shareBoardRegView.do?pageNum=${pageNum}&doctor_no=5'">글쓰기</button></div>
	

		<table width="1000" border="1" border-collapse="collapse"
			border-color="skyblue" align="center">
			<tr height="40">
				<th>번호</th>
				<th>제목</th>
				<th>작성자</th>
			...
			<c:forEach var="shareBoard" items="${list}">
				<tr>
					...
					<td class="left" width=400>
						<a class="non_line" href='shareBoardContentView.do?shareboard_no=${shareBoard.shareBoard_no }&pageNum=${currentPage}'>
							${shareBoard.shareBoard_subject}</a>
					</td>
		...
		<div class="pageNum" style="text-align: center;">
			...
			<c:if test="${endPage < pageCnt }">
				<a class="non_line2" href='shareBoardView.do?pageNum=${startPage+blockSize }'>[
					다음]</a>
			</c:if>
		</div>
